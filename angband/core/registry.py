...
class PageCacheWrite(ExploitPrimitive):
    """Page-Cache Write Primitive — the "Holy Grail" of kernel exploitation.

    Allows writing to in-memory page cache pages without triggering COW or
    writing to disk. Bypasses IMA/EVM integrity monitoring.

    Three generations in the "Dirty" family:
    - Dirty Pipe (CVE-2022-0847): Pipe subsystem
    - Copy Fail (CVE-2026-31431): AF_ALG crypto subsystem
    - Dirty Frag (CVE-2026-43284): xfrm-ESP + RxRPC chained

    Properties:
    - No disk touch (in-memory only)
    - Bypasses integrity monitoring
    - Deterministic (logic bug, no race)
    - High reliability (~100% success)
    """

    name = "page_cache_write"
    description = "Page-cache write primitive — Dirty family (splice + in-place modification)"

    def generate_c(self) -> str:
        return '''\
/* --- Page-Cache Write Primitive: Dirty family --- */
/*
 * page_cache_write: Overwrites in-memory page cache without COW or disk write.
 *
 * Three variants in the Dirty family:
 *   1. Dirty Pipe (CVE-2022-0847): pipe_buffer.flags bypasses COW check
 *   2. Copy Fail (CVE-2026-31431): AF_ALG crypto writes to splice()-injected pages
 *   3. Dirty Frag (CVE-2026-43284): xfrm-ESP + RxRPC chained for universal coverage
 *
 * Requires:
 *   - splice() syscall for page injection
 *   - Target subsystem path (IPsec/RxRPC/crypto)
 *
 * Limitations:
 *   - xfrm-ESP: needs user namespace (AppArmor blocks on Ubuntu)
 *   - RxRPC: requires fcrypt key brute-force (~2^56 options)
 *
 * Usage:
 *   1. splice() target file into pipe or socket
 *   2. Trigger subsystem-specific write path
 *   3. Page cache page now contains controlled data
 *
 * For full details, see docs/primitives.md
 */
/* --- end page_cache_write --- */
'''

    def required_headers(self) -> list[str]:
        return ["fcntl.h", "sys/splice.h"]
...
PRIMITIVE_REGISTRY: dict[str, type[ExploitPrimitive]] = {
    ...
    "page_cache_write": PageCacheWrite,
}
